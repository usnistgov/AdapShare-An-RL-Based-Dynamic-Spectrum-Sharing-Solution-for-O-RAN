...
n_seq = 1
batch_size = 128
# TimeGAN components: Network Parameters
hidden_dim = 24
num_layers = 3
...
for iteration in range(50):    
    ...
    # creating final data
    LTE_data = df
    it = int(np.ceil((len(df)-seq_len)/seq_len))
    synth_data_id = random.sample(range(generated_data.shape[0]),it+1)
    synth_data = []
    for j in range(len(synth_data_id)):
        synth_data.append(generated_data[synth_data_id[j]])
    # saving generated data to csv
    synth = {'Time':time, 'NRB': synth_data[:len(LTE_data)].flatten()}
    synth_df = pd.DataFrame(data=synth)
    synth_df.to_csv("NR_Demand_%d.tsv" %iteration,index = False, sep = '\t', encoding = 'utf-8-sig')