...
% Objective: Code corresponding to DDPG Agent
...
for n = 1:numel(N_R)
    ...
    for i = 1:numel(gamma)
        ...
        LTEFile = "Data/LTE_Demand_0.xlsx";
        NRFile = "Data/NR_Demand_0.xlsx";
    ...
    learn_ddpg(n).lte_alloc = lte_alloc;
    learn_ddpg(n).nr_alloc = nr_alloc;
    learn_ddpg(n).lte_demand = lte_demand;
    learn_ddpg(n).nr_demand = nr_demand;
    learn_ddpg(n).sur_def_lte = sur_def_lte;
    learn_ddpg(n).sur_def_nr = sur_def_nr;
    learn_ddpg(n).fairness = fairness;
end
...
for n=1:numel(N_R)
    ...
    for g = 1:numel(gamma)
        [N_avg,fval_avg,exitflag_avg] = fmincon(@(N)optim_allocation_avg(N,gamma(g),mu_lte,mu_nr,var_lte,var_nr),N_R_init,[1 1],N_R(n),[],[],lb,ub,[],options);
...
save("ddpg_comparison.mat","learn_ddpg","optimal");